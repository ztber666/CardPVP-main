      ...
      >
        ...
        >
          ...
          <div className="flex items-center gap-3">
            <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border text-xl ${a.icon}`}>{request.icon}</div>
              ...
              <div className={grid ? 'grid grid-cols-2 gap-2' : 'space-y-2'}>
                {request.options.map(o => (
                  <button
                    ...
                    onClick={e => { e.stopPropagation(); onSubmit(o.key); }}
                    className={`rounded-xl border border-card-border p-2.5 text-left transition-all duration-150 hover:border-accent-shield/40 hover:bg-card-bg/60 active:scale-[.97] disabled:pointer-events-none disabled:opacity-40 ${grid ? 'flex flex-col items-center gap-1.5' : 'flex w-full items-center gap-3'}`}
                  >
                    {o.img ? (
                    ...
                    <div className={grid ? 'text-center' : 'min-w-0 flex-1'}>
